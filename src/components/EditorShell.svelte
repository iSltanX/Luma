...
<style>
  ...
  /* ── طباعة (تصدير PDF) ────────────────────────────────────────
     `app.css` تُخفي كل ما يحمل `luma-chrome` وترفع القصّ عن `body`
     وجذر التطبيق — الشرح الكامل هناك. الباقي هنا محصور بهذا المكوّن:

     - `.surfaces` و`.panel` و`.notice`: أغلفة واجهة لا تحمل
       `luma-chrome` نفسها (طفلها وحده يحملها، أو لا طفل لها أصلًا)،
       فتبقى فارغة الشكل بحجمها المحجوز لو لم تُخفَ صراحةً هنا.
     - `.scroller`: مسؤول التمرير الوحيد (الشرح أعلى الملف) — رفع
       القصّ عنه هو ما يُخرج المستند من «صفحة ١ من ١».
     - `.sheet`: حشوتها السفلية ٤٠vh نطاق آلة كاتبة شاشي — مساحة
       فارغة يُمرَّر إليها أسفل السطر الأخير أثناء الكتابة. مطبوعةً
       تعني صفحة أخيرة شبه فارغة؛ فتُستبدل بنفس حشوتها العلوية. */
  @media print {
    .surfaces,
    .panel,
    .notice {
    ...
    .sheet {
      padding-block-end: var(--space-048);
    }
  }
</style>

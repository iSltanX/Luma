...
# ── ٠ · التدارك شرط الحذف لا لاحقُه ──────────────────────────
# `Luma.md` §٢٠ مسألة ١٩ **ثابت**: لا يُشحن الحذف بوجهيه — لا زرّ
# الحذف ولا حذف المُفرَغ عند المغادرة — قبل أن تُبنى السلّة. والثاني
# يقع بلا فعل من الكاتب فهو أولى بالقيد لا أقلّ.
#
# ولا يُترك ذلك اتفاقًا: `delete_document` محوٌ نهائي بـ`remove_dir_all`
# يمحو المستند وسجله معًا، وشحنُه بلا تدارك فقدٌ لا رجعة فيه. البوابة
# تُفتح من نفسها يوم تصل السلّة — حين يظهر حقل الحذف في مخطط التخزين.
if grep -q "pub fn delete_document" src-tauri/src/commands.rs 2>/dev/null &&
  ! grep -q "deleted_at" src-tauri/src/storage/model.rs 2>/dev/null; then
  die "الحذف مبنيٌّ والسلّة لم تُبنَ بعد — ولا يُشحن الحذف بلا تدارك.

«التدارك شرطُ الحذف لا لاحقُه» — \`Luma.md\` §٢٠ مسألة ١٩ **ثابت**،
و\`delete_document\` اليوم محوٌ نهائي يمحو المستند وسجله معًا.

تُفتح هذه البوابة من نفسها حين يدخل حقل الحذف مخطط التخزين
(\`deleted_at\` في src-tauri/src/storage/model.rs) بهجرته — أي حين
تُبنى السلّة فعلًا لا حين يُقال إنها ستُبنى."
fi
